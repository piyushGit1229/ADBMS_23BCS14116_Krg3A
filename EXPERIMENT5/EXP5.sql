...
-- Create Restricted role/user (piyush)
CREATE ROLE piyush LOGIN PASSWORD '1234';

-- piyush logs in and runs (In new query window)
SELECT * FROM v_order; -- permission denied initially

-- Grant access to piyush
GRANT SELECT ON v_order TO piyush;
SELECT * FROM v_order; -- now piyush can view

SELECT * FROM cust; --  can't view base tables

-- Revoke access from piyush
REVOKE SELECT ON v_order FROM piyush;
SELECT * FROM v_order; -- now piyush can't view
